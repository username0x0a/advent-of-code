...
$order = 'J23456789TQKA'
$values = $order.split('').each_with_index.map{|l,i| [l, i+1] }.to_h
$letters = $values.each.map{|(k,v)| [v, k] }.to_h
...
cards.each{|card|
	...
	groups = hand.split('')
		...
		.sort_by{|e|-e[0]}.to_h
	new_hand = groups.map{|k,vs| vs.map{|v| $letters[v] * k }.join('') }.reduce(:+)
	new_hand = new_hand.gsub(/[^J]/,'') + new_hand.gsub(/[J]/,'')
	new_hand = new_hand.gsub('J', new_hand.gsub(/[J]/,'')[0] || '') if new_hand.index(/[^J]/)
	groups = new_hand.split('')
		.each_with_object({}){|c,h| h[c] = 0 if !h[c]; h[c] += 1 }
		.each_with_object({}){|(l,c),h| h[c] = [] if !h[c]; h[c] << l }
		.map{|k,v| [k, v.map{|l| $values[l] }.sort.reverse] }
		.sort_by{|e|-e[0]}.to_h
